  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-start mb-8 z-10">
        ...
        <Ionicons name="ellipsis-horizontal" size={20} color={ApTheme.Color.textMuted} />
      </View>

      {/* Chart */}
      <View className="absolute bottom-0 left-0 right-0 h-40 w-full" style={{ paddingBottom: 30 }}>
        <Svg height="100%" width="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
         <Defs>
            <LinearGradient id="gradient" x1="0" y1="0" x2="0" y2="1">
              <Stop offset="0" stopColor={ApTheme.Color.primary} stopOpacity="0.5" />
              <Stop offset="1" stopColor={ApTheme.Color.primary} stopOpacity="0" />
            </LinearGradient>
          </Defs>
          <Path
      ...
      {/* X-Axis Labels */}
       <View className="flex-row justify-between absolute bottom-4 left-4 right-4">
        {LABELS.map((label, index) => (
             <ApText key={index} size="xs" color={ApTheme.Color.textMuted} style={{ fontSize: 10 }}>
                {label}
             </ApText>
        ))}
       </View>
