...
export const ConsistencyCard = ({ habits, completionRate }: Props) => {
  ...
  const dayValues = days.map((date) => {
    const eligible = activeHabits.filter((habit) => isHabitEligibleForDate(habit, date));
    if (!eligible.length) return 0;
    return eligible.filter((habit) => habit.completions?.some((completion) => isSameDateKey(completion.date, toDateKey(date)) && completion.status)).length / eligible.length;
  });
